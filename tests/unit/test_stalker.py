...
class TestStalkerPortalInit:
    ...
    def test_init_sets_headers(self):
        ...
        assert "User-Agent" in portal.headers
        assert "X-User-Agent" in portal.headers
        assert "Accept" in portal.headers
        assert "Accept-Language" in portal.headers
        assert "Referer" in portal.headers
        assert "Connection" in portal.headers
        assert portal.headers["Referer"] == "http://example.com/"
        # No manual Cookie header; cookies are in session
        assert "Cookie" not in portal.headers

    def test_init_sets_session_cookies(self):
        """Test that base cookies are set in the session."""
        portal = StalkerPortal("http://example.com", "00:11:22:33:44:55")
        cookies = portal.session.cookies
        # Check that mac, stb_lang, timezone cookies are present
        assert any(c.name == "mac" and c.value == "00:11:22:33:44:55" for c in cookies)
        assert any(c.name == "stb_lang" and c.value == "en" for c in cookies)
        assert any(c.name == "timezone" and c.value == "Europe/London" for c in cookies)
...
class TestHandshake:
    ...
    def test_handshake_success_second_path(self):
        """Test successful handshake on second path after 404 and fallback failure on first."""
        with patch("app.services.stalker.requests.Session") as mock_session_class:
            mock_session = MagicMock()
            mock_session_class.return_value = mock_session

            # First path: first get 404, fallback get also 404
            # Second path: first get returns success
            ...
            # Two 404s (path1 + fallback) then success on path2 first attempt
    ...
    def test_handshake_tries_all_three_paths(self):
        """Test that handshake tries all three paths with fallbacks."""
        with patch("app.services.stalker.requests.Session") as mock_session_class:
            mock_session = MagicMock()
            mock_session_class.return_value = mock_session
            ...
            # First two paths: each has first get 404 and fallback get 404
            # Third path (base_url): first get succeeds
            mock_session.get.side_effect = [
                mock_response_404,  # /server/load.php first attempt
                mock_response_404,  # /server/load.php fallback
                mock_response_404,  # /portal.php first attempt
                mock_response_404,  # /portal.php fallback
                mock_response_success,  # base_url first attempt
            ]
    ...
    def test_create_link(self):
        ...
        with patch("app.services.stalker.requests.Session") as mock_session_class:
            ...
            mock_response.json.return_value = {
                "js": {"cmd": "http://stream.example.com/video.ts"}
            }
            mock_session.get.return_value = mock_response
