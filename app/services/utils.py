...
def _is_hostname_forbidden(hostname):
    """
    ...
    try:
        ...
        # It's a domain name, check against localhost variants
...
def is_safe_url(url_str, allow_redirects=False):
    """
    ...
    try:
        ...
        # Block private/localhost immediately
        if _is_hostname_forbidden(hostname):
            return False

        # Optionally follow redirects and validate final destination
        if allow_redirects:
            import requests
            from app.config import settings

            try:
                # Use HEAD request with redirects to get final URL
                response = requests.head(
                    url_str, timeout=5, allow_redirects=True, verify=settings.verify_ssl
                )
                ...
                # If we can't validate redirects, be conservative and block
                return False
...
def is_safe_url_with_redirect_check(url_str):
    """
    Validate URL and follow redirects to check the final destination.
    Use this for proxy endpoints where redirects might lead to internal services.
